            ...
            if response.status_code == 200:
                ...
                if ai_providers:
                    for provider, status in ai_providers.items():
                        print(
                            f"  - {provider}: configured={status.get('configured', False)}, active={status.get('active', False)}"
                        )
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"  - Default Model: {anthropic_config.get('defaultModel', 'N/A')}"
                )
        ...
        try:
            ...
            test_payload = {
                ...
                "config": {
                    ...
                    "baseUrl": anthropic_config.get(
                        "baseUrl", "https://api.anthropic.com"
                    ),
                    "defaultModel": anthropic_config.get(
            ...
            if response.status_code == 200:
                ...
                if result.get("success"):
                    print(
                        f"✓ {result.get('message', 'Anthropic connection successful!')}"
                    )
                    return True
            ...
            print(
                "✗ Request timed out - this might indicate the API is trying to connect"
            )
            return False
        except Exception as e:
