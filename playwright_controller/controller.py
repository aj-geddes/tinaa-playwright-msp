...
class PlaywrightController:
    ...
    async def take_screenshot(self, name: str, selector: str = None, full_page: bool = False) -> Optional[str]:
    ...
    async def fill_form(self, fields: Dict[str, str], submit_selector: str = None) -> bool:
    ...
    async def check_responsive_design(self, breakpoints: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def test_login_form(self, username_selector: str, password_selector: str, 
    ...
    async def _check_login_success(self) -> bool:
        ...
        has_logout = await self.page.query_selector('a:text("Logout"), a:text("Log out"), button:text("Logout"), button:text("Log out")')
        if has_logout:
            return True
        
        # Check for common error messages
        has_error = await self.page.query_selector('text="Invalid username", text="Invalid password", text="Login failed", text="Incorrect credentials"')
        if has_error:
            return False
    ...
    async def extract_form_fields(self, form_selector: str = None) -> List[Dict[str, Any]]:
    ...
    async def run_security_checks(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for form in forms:
                if form.get("hasPassword") and form.get("action", "").startswith("http:"):
                    self.findings["high"].append({
                        "type": "security",
    ...
    async def get_test_report(self, test_type: str, url: str) -> Dict[str, Any]:
        ...
        # Format findings
        high_priority = [f"{i+1}. {issue['description']}" for i, issue in enumerate(self.findings["high"])]
        medium_priority = [f"{i+1}. {issue['description']}" for i, issue in enumerate(self.findings["medium"])]
        low_priority = [f"{i+1}. {issue['description']}" for i, issue in enumerate(self.findings["low"])]
        ...
        if self.findings["high"] or self.findings["medium"]:
            next_steps.append("1. Address the identified issues starting with high-priority items")
            next_steps.append("2. Run focused tests on problematic areas after fixes")
        else:
            next_steps.append("1. Run additional test types to ensure comprehensive coverage")
        
        next_steps.append("3. Consider implementing automated testing for regression prevention")
        ...
        for i, screenshot in enumerate(self.screenshots[:5]):  # Limit to 5 screenshots
            screenshots_formatted.append(f"{i+1}. {screenshot['name']} ({screenshot['type']})")
        
        # Generate summary
        summary = f"Tested {url} using {test_type} methodology. "
        
        if not self.findings["high"] and not self.findings["medium"] and not self.findings["low"]:
            summary += "No issues were found."
        else:
            total_issues = len(self.findings["high"]) + len(self.findings["medium"]) + len(self.findings["low"])
