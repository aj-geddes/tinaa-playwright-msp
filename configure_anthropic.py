...
async def configure_anthropic():
    ...
    if use_defaults == 'n':
        base_url = input("Base URL (default: https://api.anthropic.com): ") or "https://api.anthropic.com"
        model = input("Default model (default: claude-3-sonnet-20240229): ") or "claude-3-sonnet-20240229"
    else:
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"API Key: {'Configured' if anthropic.get('apiKey') else 'Not set'}")
...
async def main():
    ...
    if is_configured:
        reconfigure = input("\nAnthropic is already configured. Reconfigure? (y/N): ").lower()
        if reconfigure != 'y':
