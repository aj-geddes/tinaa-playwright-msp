...
class AIManager:
    ...
    async def initialize_from_secrets(self):
        ...
        if validation_results.get("anthropic"):
            try:
                anthropic_config = await secrets_manager.get_ai_provider_config("anthropic")
    ...
    async def _cache_playbook(self, playbook: Dict[str, Any], project_id: Optional[str]):
        """Cache generated playbook"""
        cache_file = self.ai_cache_path / f"playbook_{project_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        async with aiofiles.open(cache_file, 'w') as f:
            await f.write(json.dumps(playbook, indent=2))
            
    async def _cache_troubleshooting(self, project_id: str, data: Dict[str, Any]):
        """Cache troubleshooting results"""
        cache_file = self.ai_cache_path / f"troubleshooting_{project_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        async with aiofiles.open(cache_file, 'w') as f:
            await f.write(json.dumps(data, indent=2))
            
    async def _cache_optimization(self, project_id: str, data: Dict[str, Any]):
        """Cache optimization results"""
        cache_file = self.ai_cache_path / f"optimization_{project_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        async with aiofiles.open(cache_file, 'w') as f:
            await f.write(json.dumps(data, indent=2))
        ...
        return {
            "providers": {
                name: {
                    ...
                    "config": {k: "***" if "key" in k.lower() else v for k, v in provider.config.items()}
                }
