...
async def create_project(
    ...
    try:
        ...
        if ctx and result.get("success"):
            ...
            await ctx.error(f"Failed to create project: {result.get('error', 'Unknown error')}")
...
async def create_project_from_url(
    ...
    try:
        ...
        if ctx and result.get("success"):
            ...
            await ctx.error(f"Failed to create project from URL: {result.get('error', 'Unknown error')}")
...
async def get_workspace_status(ctx: Context = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if ctx:
            await ctx.success(f"Workspace status retrieved: {total_projects} projects, {active_projects} active")
...
async def clone_repository(
    ...
    try:
        ...
        destination_path = workspace_manager.projects_path / destination_name
        ...
        if result.get("success") and ctx:
            ...
            await ctx.error(f"Failed to clone repository: {result.get('error', 'Unknown error')}")
...
async def get_repository_info(
    ...
    try:
        ...
        if result.get("success") and ctx:
            await ctx.success(f"Repository information retrieved: {result.get('name', 'Unknown')}")
        elif ctx:
            await ctx.error(f"Failed to get repository info: {result.get('error', 'Unknown error')}")
