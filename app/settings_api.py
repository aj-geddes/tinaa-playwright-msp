...
async def get_current_credentials() -> Dict[str, Any]:
    ...
    try:
        ...
        config = {
            "openai": {
                "enabled": await secrets_manager.has_secret(
                    "tinaa-openai-secret", "api_key"
                ),
                "baseUrl": await secrets_manager.get_secret(
                    "tinaa-openai-secret", "base_url"
                )
                or "https://api.openai.com/v1",
                "defaultModel": await secrets_manager.get_secret(
                    "tinaa-openai-secret", "default_model"
                )
                or "gpt-4",
                "apiKey": (
                    "••••••••"
                    if await secrets_manager.has_secret(
                        "tinaa-openai-secret", "api_key"
                    )
                    else ""
                ),
            ...
            "anthropic": {
                "enabled": await secrets_manager.has_secret(
                    "tinaa-anthropic-secret", "api_key"
                ),
                "baseUrl": await secrets_manager.get_secret(
                    "tinaa-anthropic-secret", "base_url"
                )
                or "https://api.anthropic.com",
                "defaultModel": await secrets_manager.get_secret(
                    "tinaa-anthropic-secret", "default_model"
                )
            ...
            "ollama": {
                ...
                "defaultModel": await secrets_manager.get_secret(
                    "tinaa-ollama-secret", "default_model"
                )
                or "llama2",
            },
            "git": {
                "pat": {
                    "enabled": await secrets_manager.has_secret(
                        "tinaa-git-pat-secret", "token"
                    ),
                    "username": await secrets_manager.get_secret(
                        "tinaa-git-pat-secret", "username"
                    )
                    or "git",
                    "token": (
                        "••••••••"
                ...
                "githubApp": {
                    "enabled": await secrets_manager.has_secret(
                        "tinaa-git-app-secret", "app_id"
                    ),
                    "appId": await secrets_manager.get_secret(
                        "tinaa-git-app-secret", "app_id"
                    )
                    or "",
                    "installationId": await secrets_manager.get_secret(
                        "tinaa-git-app-secret", "installation_id"
                    )
                    or "",
                    "privateKey": (
                        "••••••••"
                        if await secrets_manager.has_secret(
                            "tinaa-git-app-secret", "private_key"
                        )
                        else ""
                    ),
                },
        ...
        raise HTTPException(
            status_code=500, detail="Failed to load credentials configuration"
        )
...
async def save_credentials(config: CredentialConfig) -> Dict[str, Any]:
    ...
    try:
        ...
        if config.anthropic and config.anthropic.enabled and config.anthropic.apiKey:
            await secrets_manager.create_or_update_secret(
                "tinaa-anthropic-secret",
                {
                    ...
                    "default_model": config.anthropic.defaultModel
                    or "claude-3-sonnet-20240229",
                },
            )
        ...
        if (
            config.git
            and config.git.pat
            and config.git.pat.enabled
            and config.git.pat.token
        ):
            await secrets_manager.create_or_update_secret(
        ...
        if (
            config.git
            and config.git.githubApp
            and config.git.githubApp.enabled
            and config.git.githubApp.appId
            and config.git.githubApp.privateKey
        ):
            await secrets_manager.create_or_update_secret(
...
async def _test_openai_credentials(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        result = await provider.chat_completion(
            "Hello, respond with just 'OK' to confirm connection."
        )
...
async def _test_github_app_credentials(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not all([app_id, installation_id, private_key]):
            return {
                "success": False,
                "message": "App ID, Installation ID, and Private Key are required",
            }
...
async def get_provider_status() -> Dict[str, Any]:
    ...
    """
    try:
        status = {"ai_providers": {}, "git_auth": {}, "last_checked": None}
