...
class GitAuthenticator:
    ...
    async def get_authenticated_clone_url(
        ...
        elif git_config["auth_type"] == "github_app":
    ...
    async def _get_pat_clone_url(
        ...
        if repo_info["provider"] == "github":
            ...
            authenticated_url = repository_url.replace("https://", f"https://{username}:{token}@")
    ...
    async def _get_github_app_clone_url(
        ...
        repo_info = self._parse_repository_url(repository_url)
        if repo_info["provider"] != "github":
            raise Exception("GitHub App authentication only supports GitHub repositories")
    ...
    async def _get_github_app_token(self, credentials: Dict[str, str]) -> Dict[str, Any]:
        ...
        if cache_key in self.token_cache:
            ...
            if datetime.fromisoformat(cached_token["expires_at"]) > datetime.now() + timedelta(minutes=5):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(url, headers=headers) as response:
                if response.status != 201:
                    ...
                    raise Exception(f"Failed to get GitHub App token: {response.status} - {error_text}")
    ...
    async def clone_repository(
        ...
        try:
            ...
            auth_url, auth_info = await self.get_authenticated_clone_url(repository_url, auth_type)
    ...
    async def _fetch_gitlab_repo_info(
        ...
        return {
            "success": False,
            "error": "GitLab API integration not yet implemented"
        }
    ...
    async def validate_git_credentials(self, auth_type: str = "auto") -> Dict[str, Any]:
        ...
        try:
            ...
            if not git_config.get("auth_type"):
                return {
                    "valid": False,
                    "error": "No git authentication configured"
                }
            ...
            try:
                auth_url, auth_info = await self.get_authenticated_clone_url(test_repo, auth_type)
...
async def clone_repository(repository_url: str, destination_path: str, **kwargs) -> Dict[str, Any]:
    """Convenience function for cloning repositories"""
    return await git_authenticator.clone_repository(repository_url, destination_path, **kwargs)
