...
class ProgressTracker:
    ...
    async def update(self, message: str, level: ProgressLevel = ProgressLevel.INFO, 
                    progress: Optional[float] = None, **metadata):
        ...
        logger.log(
            ...
            f"[Progress] {message}" + (f" ({progress}%)" if progress is not None else "")
        )
...
class ExploratoryTestProgress(ProgressTracker):
    ...
    async def element_tested(self, element_type: str, index: int, total: int):
        ...
        await self.info(
            f"Testing {element_type} ({index}/{total})",
            progress=progress
        )
