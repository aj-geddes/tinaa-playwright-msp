...
class SecretsManager:
    ...
    async def _get_kubernetes_secret(self, secret_name: str, key: str) -> Optional[str]:
        ...
        try:
            ...
            logger.error(f"Failed to get Kubernetes secret {secret_name}:{key}: {str(e)}")
            return None
    
    async def _get_secret_via_kubernetes_api(self, secret_name: str, key: str) -> Optional[str]:
        ...
        try:
            ...
            logger.error(f"Failed to get secret via K8s API {secret_name}:{key}: {str(e)}")
    ...
    def _get_environment_secret(self, secret_name: str, key: str) -> Optional[str]:
        ...
        vault_mappings = {
            ...
            ("tinaa-anthropic-secret", "default-model"): "TINAA_ANTHROPIC_DEFAULT_MODEL",
            ...
            ("tinaa-git-app-secret", "installation-id"): "TINAA_GIT_APP_INSTALLATION_ID",
    ...
    async def get_ai_provider_config(self, provider_name: str) -> Dict[str, Any]:
        ...
        if provider_name == "openai":
            ...
            if api_key:
                ...
                default_model = await self.get_secret("tinaa-openai-secret", "default-model")
                
                config = {
                    "api_key": api_key,
                    "base_url": base_url or os.getenv("TINAA_OPENAI_BASE_URL") or os.getenv("OPENAI_BASE_URL", "https://api.openai.com/v1"),
                    "default_model": default_model or os.getenv("TINAA_OPENAI_DEFAULT_MODEL") or os.getenv("OPENAI_DEFAULT_MODEL", "gpt-4"),
                    "max_tokens": int(os.getenv("TINAA_OPENAI_MAX_TOKENS") or os.getenv("OPENAI_MAX_TOKENS", "4096")),
                    "temperature": float(os.getenv("TINAA_OPENAI_TEMPERATURE") or os.getenv("OPENAI_TEMPERATURE", "0.1"))
                }
            ...
            if api_key:
                ...
                default_model = await self.get_secret("tinaa-anthropic-secret", "default-model")
                
                config = {
                    "api_key": api_key,
                    "base_url": base_url or os.getenv("TINAA_ANTHROPIC_BASE_URL") or os.getenv("ANTHROPIC_BASE_URL", "https://api.anthropic.com"),
                    "default_model": default_model or os.getenv("TINAA_ANTHROPIC_DEFAULT_MODEL") or os.getenv("ANTHROPIC_DEFAULT_MODEL", "claude-3-sonnet-20240229"),
                    "max_tokens": int(os.getenv("TINAA_ANTHROPIC_MAX_TOKENS") or os.getenv("ANTHROPIC_MAX_TOKENS", "4096")),
                    "temperature": float(os.getenv("TINAA_ANTHROPIC_TEMPERATURE") or os.getenv("ANTHROPIC_TEMPERATURE", "0.1"))
                }
    ...
    async def _get_github_app_config(self) -> Optional[Dict[str, Any]]:
        ...
        installation_id = await self.get_secret("tinaa-github-app-secret", "installation-id")
        ...
        if not installation_id:
            installation_id = await self.get_secret("github-app-secret", "installation-id")
    ...
    async def create_or_update_secret(self, secret_name: str, data: Dict[str, str]) -> bool:
        """
    ...
    async def _create_kubernetes_secret(self, secret_name: str, data: Dict[str, str]) -> bool:
        ...
        try:
            ...
            for key, value in data.items():
                ...
                encoded_data[k8s_key] = base64.b64encode(value.encode('utf-8')).decode('utf-8')
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.put(api_url, json=secret_payload, headers=headers, ssl=False) as response:
                    if response.status in [200, 201]:
                        logger.info(f"Successfully updated Kubernetes secret: {secret_name}")
                        # Clear cache for this secret
                        keys_to_remove = [k for k in self.secrets_cache.keys() if k.startswith(f"{secret_name}:")]
                        ...
                        async with session.post(create_url, json=secret_payload, headers=headers, ssl=False) as create_response:
                            if create_response.status in [200, 201]:
                                logger.info(f"Successfully created Kubernetes secret: {secret_name}")
                                return True
                            else:
                                error_text = await create_response.text()
                                logger.error(f"Failed to create secret {secret_name}: {create_response.status} - {error_text}")
                                return False
                    else:
                        error_text = await response.text()
                        logger.error(f"Failed to update secret {secret_name}: {response.status} - {error_text}")
                        return False
            
        except Exception as e:
            logger.error(f"Exception creating Kubernetes secret {secret_name}: {str(e)}")
            return False
    ...
    def get_environment_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "has_service_account": os.path.exists("/var/run/secrets/kubernetes.io/serviceaccount/token"),
