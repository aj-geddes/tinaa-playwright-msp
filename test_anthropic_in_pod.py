    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            test_payload = {
                ...
                "context": {"test_type": "integration test", "issue": "flaky tests"},
            }

            response = await client.post(
                f"{base_url}/api/v1/internal/problem-solving", json=test_payload
            )
        ...
        try:
            ...
            response = await client.post(
                f"{base_url}/api/v1/collaborative/start-session", json=session_payload
            )
            ...
            if response.status_code == 200:
                ...
                if session.get("session_id") and session.get("initial_questions"):
                    ...
                    print(
                        f"   - Questions generated: {len(session.get('initial_questions', []))}"
                    )
                else:
